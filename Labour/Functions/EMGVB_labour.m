classdef EMGVB_labour < VBayesLab
    ...
    methods
        function obj = EMGVB_labour(mdl,data,varargin)
...
%             Y12 = grad_clipping(Y12,max_grad_init);
            ...
            Y12 = grad_clipping([gradLB_mu;gradLB_iSig(:)],max_grad_init);
            gradLB_mu = Y12(1:d_theta);
            if ~isDiag
                gradLB_iSig = reshape(Y12(d_theta+1:end),d_theta,d_theta);
            else
                gradLB_iSig = Y12(d_theta+1:end);
            end
...
%                     end
                ...
                Y12 = grad_clipping([gradLB_mu;gradLB_iSig(:)],max_grad);
                gradLB_mu = Y12(1:d_theta);
                if ~isDiag
                    gradLB_iSig = reshape(Y12(d_theta+1:end),d_theta,d_theta);
                else
                ...
                train.perf_theta(iter,:)= perf_measures(data.train,mu);
                ...
                test.perf_theta(iter,:) =  perf_measures(data.test,mu);
            ...
            Post.train.perf_best = perf_measures(data.train,Post.mu);
            Post.test.perf_best  = perf_measures(data.test,Post.mu);
            Post.perf_short      = [Post.train.LB_max, Post.train.ll_best, Post.train.perf_best,...
                                    Post.test.LB_max,  Post.test.ll_best,  Post.test.perf_best];
            Post.par = mu';
