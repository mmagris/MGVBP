classdef MGVB_labour < VBayesLab
    ...
    methods
        function obj = MGVB_labour(mdl,data,varargin)
...
%             Y12 = grad_clipping(Y12,max_grad_init);
            ...
            Y12 = grad_clipping([gradLB_mu;gradLB_Sig(:)],max_grad_init);
            gradLB_mu = Y12(1:d_theta);
            if ~isDiag
                gradLB_Sig = reshape(Y12(d_theta+1:end),d_theta,d_theta);
            else
                gradLB_Sig = Y12(d_theta+1:end);
            end
...
%                     end
                ...
                Y12 = grad_clipping([gradLB_mu;gradLB_Sig(:)],max_grad);
                gradLB_mu = Y12(1:d_theta);
                if ~isDiag
                    gradLB_Sig = reshape(Y12(d_theta+1:end),d_theta,d_theta);
                else
